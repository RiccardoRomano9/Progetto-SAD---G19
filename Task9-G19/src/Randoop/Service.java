...
public class Service {
	public static void main(String[] args) throws IOException {
		File class_file = new File("C:\\Users\\hp\\Downloads\\minimised_classes_2016\\subjects\\anonymised\\FontInfo.java");
    	int cov;
    	int ex_cov = 0;
    	int timelimit = 5;
    	int max_iter = 500;
    	...
    	File director = new File((class_file.getName().subSequence(0,class_file.getName().length()-5)+"_Livelli"));
		director.mkdir();
    	...
    	do {
    		...
    		if(ex_cov == cov) {
    			...
    			File directory = new File(".\\" + class_file.getName().subSequence(0,class_file.getName().length()-5)+"_Livelli/livello"+livello);
    			directory.mkdir();
    			
    			Path destinationFile = Path.of(directory.getPath());
    			System.out.println(destinationFile);
    			
    			for(int i=0; i<=iter; i++) {
    				File sourceFile = new File(".\\"+class_file.getName().subSequence(0,class_file.getName().length()-5)+"-"+i+"-dati_di_copertura\\"+class_file.getName().subSequence(0,class_file.getName().length()-5)+"_Test");
        	            ...
        	            for (File f : files) {
        	                if (f.isFile()) {
        	                    ...
        	                    File nuovoFile = new File(f.getParent(), nuovoNome);
        	                        ...
        	                        Path destinazione = new File(directory.getPath(), nuovoNome).toPath();
